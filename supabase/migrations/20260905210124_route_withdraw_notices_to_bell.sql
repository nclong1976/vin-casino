-- Theo yêu cầu: 2 thông báo tự động của luồng RÚT TIỀN ("Yêu cầu rút tiền
-- đang chờ phê duyệt" tạo phía client trong WithdrawModal.jsx, và "Biến
-- động số dư: -..." tạo ở đây khi Admin duyệt) không còn gửi vào khung chat
-- CSKH (bảng messages) nữa - chuyển sang chuông thông báo riêng của người
-- dùng (bảng notifications, theo user_id) để không làm loãng cuộc trò
-- chuyện thật với CSKH. CHỈ đổi ĐÍCH insert cho đúng 1 trường hợp duy nhất
-- (p_action = 'approve' và v_tx.type = 'withdraw') - nạp tiền, từ chối
-- nạp/rút vẫn gửi vào messages y hệt cũ, không đổi logic nghiệp vụ/khoá/
-- điều kiện nào khác.
...
begin
  ...
  if p_action = 'approve' and v_tx.type = 'withdraw' then
    ...
    -- vì khung chat CSKH (xem ghi chú đầu file migration này).
