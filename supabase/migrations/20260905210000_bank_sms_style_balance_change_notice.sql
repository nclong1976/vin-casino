-- Đổi cấu trúc mẫu thông báo "Biến động số dư" (khi Admin DUYỆT nạp/rút
-- tiền - 2 trường hợp duy nhất còn dùng tiêu đề này, các thông báo "đang
-- chờ phê duyệt"/"bị từ chối" không đổi) sang đúng cấu trúc SMS Brandname
-- ngân hàng theo yêu cầu:
--   TK {tài khoản đăng nhập}: {+/-}{Amount} VND luc {Time} {Date}.
--   SD: {Current_Balance} VND. ND: {Transaction_Content}
-- "tài khoản" ở đây là ID/tên đăng nhập của người dùng (users.identifier -
-- luôn có sẵn từ lúc đăng ký, fallback về phần trước "@" của email rồi
-- tới id nếu identifier trống). ND dùng nội dung chung chung kiểu ngân
-- hàng thật, không kèm mã giao dịch (đã xác nhận qua AskUserQuestion).
-- CHỈ đổi v_title/v_content của 2 nhánh DUYỆT nạp + DUYỆT rút - không đổi
-- logic nghiệp vụ/khoá/điều kiện/đích insert (messages/notifications) nào
-- khác so với migration trước.
...
begin
  ...
  if p_action = 'approve' and v_tx.type = 'withdraw' then
    -- Duyệt RÚT tiền: gửi thẳng chuông thông báo riêng của người dùng thay
    -- vì khung chat CSKH (xem ghi chú migration trước).
    ...
    values (
      ...
      v_tx.user_id, v_title, v_content, 'withdraw', false, now()
    );
  else
