...
export default function BaiCao() {
  ...
  return (
    <main className="relative w-full max-w-[480px] mx-auto min-h-screen bg-[#0c0905] overflow-clip font-heading flex flex-col">
      ...
      {/* Header */}
      <header className="sticky top-0 z-40 flex items-center justify-between px-4 py-3 bg-[#120d08]/95 backdrop-blur border-b border-[#3a2c14]">
        <Link to="/casino" className="p-1 -m-1 active:scale-90 transition-transform">
          <ArrowLeft className="w-4 h-4 text-[#e8c87a]" />
        </Link>
        <h1 className="text-[13px] font-semibold tracking-[0.15em] text-[#e8c87a]">BÀI CÀO</h1>
      ...
      {/* Balance bar */}
      <div className="relative z-10 flex items-center justify-between px-4 py-2.5 bg-[#160f08] border-b border-[#3a2c14]">
        <div className="flex items-center gap-1.5">
          <Coins className="w-3.5 h-3.5 text-[#e8c87a]" />
          <span className="text-[11px] font-semibold text-[#e8c87a]">{fmt(balance)} VNĐ</span>
        </div>
        <button onClick={resetBalance} className="text-[9px] text-[#9a8456] active:opacity-70 flex items-center gap-1">
          <RefreshCw className="w-2.5 h-2.5" /> Nạp lại
        </button>
      </div>

      {/* Table */}
      <div className="relative z-10 flex-1 flex flex-col items-center justify-between py-5 px-3">
        {/* Dealer */}
      ...
      {/* Controls */}
      <div className="relative z-10 px-4 pb-20 pt-2 bg-gradient-to-t from-[#0c0905] to-transparent">
        {phase === "betting" && (
